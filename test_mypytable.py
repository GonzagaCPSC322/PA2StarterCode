...
# extended join practice problem
data_car_left_extra_long = [[555.0,12.0,"ford pinto",75.0,3076.0], # full match
    [556.0,12.0,"toyota truck",79.0,2989.0], # 0 match
    [557.0,12.0,"toyota corolla",75.0,2611.0], # full match
    [996.0,13.0,"toyota corolla",75.0,2800.0], # full match
    [997.0,12.0,"toyota corolla",76.0,2989.0], # match on CarName, match on ModelYear; not together
    [998.0,12.0,"ford pinto",74.0,2989.0], # match on CarName, 0 match on ModelYear
    [999.0,12.0,"ford mustang",77.0,2989.0], # 0 match on CarName, match on ModelYear
    [1000.0,11.0,"amc ambassador sst",72.0,3902.0]] # full match
data_car_right_extra_long = [["honda accord",75.0,2789.0], # 0 match on CarName, match on ModelYear
    ["ford pinto",75.0,2769.0], # full match
    ["toyota corolla",75.0,2711.0], # full match
    ["ford pinto",76.0,3025.0], # match on CarName, match on ModelYear; not together
    ["toyota corolla",77.0,2789.0], # match on CarName, match on ModelYear; not together
    ["range rover",70.0,3333.0], # 0 match
    ["ford pinto",73.0,2567.0], # match on CarName, 0 match on ModelYear
    ["toyota corolla",75.0,2999.0], # full match
    ["amc ambassador sst",72.0,3000.0]] # full match
...
header_NAs2 = ["id", "a", "b", "c"]
data_NAs2 = [["ID001", 3.5, 5, 1.1],
            ["ID002", 2.2, 7, -5.3],
            ["ID003", 1.0, "NA", 20.20],
            ["ID004", "NA", -5, 100.101],
            ["ID005", 1.4, 5, "NA"],
...
def test_get_column():
    ...
    for i, col_label in enumerate(header):
        ...
        for j, val in enumerate(col):
            assert val == data_as_numeric[j][i]
...
def test_save_to_file():
    ...
    # delete saved file from prev run if it exists
...
def test_find_duplicates():
    ...
    assert not row_indexes_to_drop

def test_remove_rows_with_missing_values():
    """Test function
    """
    # one row with missing value
    ...
    # several rows of missing values
    table = MyPyTable(header_NAs2, data_NAs2)
    table.remove_rows_with_missing_values()
    assert len(table.data) == 2
    removed_nas2 = [["ID001", 3.5, 5, 1.1],
            ["ID002", 2.2, 7, -5.3]]
    assert table.data == removed_nas2
...
def fill_df_missing_values(df, fill_str="NA"):
    """Utility function
    """
    df_joined_as_list = df.values.tolist()
    for i, _ in enumerate(df_joined_as_list):
        for j, _ in enumerate(df_joined_as_list[i]):
            df_joined_as_list[i][j] = fill_str if pd.isnull(df_joined_as_list[i][j]) \
                else df_joined_as_list[i][j]
    return df_joined_as_list
...
def test_perform_full_outer_join():
    ...
    df_joined_as_list = fill_df_missing_values(df_joined)
    check_same_lists_regardless_of_order(joined_table.data, df_joined_as_list)
    ...
    # extended join practice problem
    table_left = MyPyTable(header_car_left_long, data_car_left_extra_long)
    table_right = MyPyTable(header_car_right_long, data_car_right_extra_long)
    joined_table = table_left.perform_full_outer_join(table_right, ["CarName", "ModelYear"])
    assert len(joined_table.column_names) == 6
    # test against pandas' outer join
    df_left = pd.DataFrame(data_car_left_extra_long, columns=header_car_left_long)
    df_right = pd.DataFrame(data_car_right_extra_long, columns=header_car_right_long)
    df_joined = df_left.merge(df_right, how="outer", on=["CarName", "ModelYear"])
    df_joined_as_list = fill_df_missing_values(df_joined)
    check_same_lists_regardless_of_order(joined_table.data, df_joined_as_list)
